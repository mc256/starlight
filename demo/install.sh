...
then
  # NO
  sed "s/STARLIGHT_PROXY/--plain-http --server=$proxy_address/" demo/starlight.service > /lib/systemd/system/starlight.service
else
  # YES
  sed "s/STARLIGHT_PROXY/--server=$proxy_address/" demo/starlight.service > /lib/systemd/system/starlight.service
fi
