#!/bin/bash
# This script converts typical container image to starlight container image
# It requires a modified version of stargz-snapshotter.
# 1. checkout git@github.com:mc256/stargz-snapshotter.git branch master_2
# 2. compile and install this version of stargz
# 3. 

REGISTRY="192.168.1.197:5002"
...
for VAL in "${ImageList[@]}"; do
  ...
  --mount type=bind,src=$PWD/config/entrypoint-hello.sh,dst=/entrypoint.sh,options=rbind:ro \
...
for VAL in "${PYLIST[@]}"; do
  ...
  mkdir /tmp/t1
  ctr-remote image optimize --plain-http \
      --mount type=bind,src=$PWD/config/entrypoint-py.sh,dst=/entrypoint.sh,options=rbind:ro \
      --mount type=bind,src=$PWD/config/scripts,dst=/app,options=rbind:rw \
      --entrypoint='[ "/entrypoint.sh" ]' \
    "$VAL" "http://$REGISTRY/$VAL-starlight"
...
for VAL in "${NODELIST[@]}"; do
  ...
  mkdir /tmp/t1
  ctr-remote image optimize --plain-http \
      --mount type=bind,src=$PWD/config/entrypoint-js.sh,dst=/entrypoint.sh,options=rbind:ro \
      --mount type=bind,src=$PWD/config/scripts,dst=/app,options=rbind:rw \
      --entrypoint='[ "/entrypoint.sh" ]' \
    "$VAL" "http://$REGISTRY/$VAL-starlight"
...
for VAL in "${JAVALIST[@]}"; do
  ...
  mkdir /tmp/t1
  ctr-remote image optimize --plain-http \
      --mount type=bind,src=$PWD/config/entrypoint-java.sh,dst=/entrypoint.sh,options=rbind:ro \
      --mount type=bind,src=$PWD/config/scripts,dst=/app,options=rbind:rw \
      --entrypoint='[ "/entrypoint.sh" ]' \
    "$VAL" "http://$REGISTRY/$VAL-starlight"
...
for VAL in "${GOLANGLIST[@]}"; do
  ...
  mkdir /tmp/t1
  ctr-remote image optimize --plain-http \
      --mount type=bind,src=$PWD/config/entrypoint-go.sh,dst=/entrypoint.sh,options=rbind:ro \
      --mount type=bind,src=$PWD/config/scripts,dst=/app,options=rbind:rw \
      --entrypoint='[ "/entrypoint.sh" ]' \
    "$VAL" "http://$REGISTRY/$VAL-starlight"
...
for VAL in "${memcachedlist[@]}"; do
  ...
  ctr-remote image optimize --plain-http \
    --env-file=$PWD/config/all.env \
    --mount type=bind,src=$PWD/config/entrypoint-memcached.sh,dst=/entrypoint.sh,options=rbind:ro \
	  --wait-on-signal \
	  --entrypoint='[ "/entrypoint.sh" ]' \
	"$VAL" "http://$REGISTRY/$VAL-starlight"
